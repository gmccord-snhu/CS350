...
#    2          Added code for assignment
#               Added code to resolve run time errors
#               Added extensive debugging to track progress
#               Added routine to display start up message
#------------------------------------------------------------------
...
##
## DEBUG State Machine flag - boolean value to indicate whether or not to print
## status messages related to state machine events and transitions
##
DEBUGSM = True
...
class CWMachine(StateMachine):
    ...
    ##---------------------------------------------------------------
    ## Debugging calls for state machine
    ## -->

    ##
    ## on_doDot - Action performed when the doDot event is called
    ##
    def on_doDot(self):
        if DEBUGSM:
            print(f"--- Event Triggered: doDot (Current State: {self.current_state.id}) ---")

    ##
    ## on_doDash - Action performed when the doDash event is called
    ##
    def on_doDash(self):
        if DEBUGSM:
            print(f"--- Event Triggered: doDash (Current State: {self.current_state.id}) ---")

    ##
    ## on_doDDP - Action performed when the doDDP event is called (Dot/Dash Pause)
    ##
    def on_doDDP(self):
        if DEBUGSM:
            print(f"--- Event Triggered: doDDP (Current State: {self.current_state.id}) ---")

    ##
    ## on_doLP - Action performed when the doLP event is called (Letter Pause)
    ##
    def on_doLP(self):
        if DEBUGSM:
            print(f"--- Event Triggered: doLP (Current State: {self.current_state.id}) ---")

    ##
    ## on_doWP - Action performed when the doWP event is called (Word Pause)
    ##
    def on_doWP(self):
        if DEBUGSM:
            print(f"--- Event Triggered: doWP (Current State: {self.current_state.id}) ---")

    ## <--
    ## end of debugging calls
    ##---------------------------------------------------------------
    ...
    def on_enter_dot(self):
        ...
        if DEBUG:
            print("* dot '.', RED, [ON], 500ms")
        self.redLight.on()
        sleep(0.5)

        # Force transition back to 'off'. Handle the state change here because the
        # preceding sleep(1.5) is a blocking operation. This synchronous return to
        # the 'off' state is required to prevent a **TransitionNotAllowed** exception
        # when the a 'transmit' loop immediately attempts to process the next Morse
        # code element, as all events are only valid from the 'off' state.
        self.doDot()
    ...
    def on_enter_dotDashPause(self):
        ...
        # Explicitly transition back to 'off' state
        self.doDDP()
    ...
    def on_enter_letterPause(self):
        # wait for 750ms
        sleep(0.75)
        # (TODO: completed by the sleep above)
    ...
    def toggleMessage(self):
        ...
        if(DEBUG):
            print(f"* Toggling message CURRENT: {self.activeMessage} ")

        # Logic to toggle between SOS and OK
        ...
        if(DEBUG):
            print(f"* Toggling message NEW: {self.activeMessage} ")
    ...
    def processButton(self):
        ...
        # LCD feedback: show new active message
        self.screen.updateScreen(f"Message changed:\n{self.activeMessage}")

        # LED feedback: quick flash sequence
        for _ in range(2):  # flash twice
            self.redLight.on()
            self.blueLight.on()
            sleep(0.15)  # on for 150ms
            self.redLight.off()
            self.blueLight.off()
            sleep(0.15)  # off for 150ms

        # Optional DEBUG prints
        if DEBUG:
            print("\n[BUTTON pressed]\n")
            print(f"Active message is now: {self.activeMessage}\n")
            print("[LEDs flashed to indicate button press]\n")
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if x == ".":
                            ...
                            if DEBUG:
                                print(f"Warning: Unknown Morse symbol '{x}'")

                        # Pause between dots/dashes if more symbols remain
                        if morseCounter < lenMorse:
                            self.doDDP()
                            morseCounter += 1  # increment counter

                    # After all symbols in the letter are done, pause between letters if needed
                    if wordCounter < lenWord:
                        self.doLP()
                        wordCounter += 1  # move to next letter

                # After all letters in the word are done, pause between words if needed
                if wordsCounter < lenWords:
                    self.doWP()
                    wordsCounter += 1  # move to next word
    ...
    def startCountdown(self):
        """
        Display a short countdown on the 16x2 LCD before starting Morse code transmission.
        This is a 16x2 display so have to be careful with line lengths
        """

        # Inform the user that the countdown is starting
        self.screen.updateScreen("STARTing in\n2 seconds..")
        if DEBUG:
            print("STARTing in 2 sec..")
        sleep(2)  # wait 2 seconds

        # Countdown numbers from 2 → 1
        for i in [2, 1]:
            self.screen.updateScreen(f"Countdown:\n{i}")
            if DEBUG:
                print(i)
            sleep(1)  # wait 1 second

        # Step 3: Final "START" message
        self.screen.updateScreen("Transmission\nSTART")
        if DEBUG:
            print("START")  # console debug
        # short pause before beginning Morse code otherwise user won't see it
        sleep(1)
...
##
## Run new method to show user countdown to start
##
cwMachine.startCountdown()
...
##
## Hook green button press event into machine event
##
